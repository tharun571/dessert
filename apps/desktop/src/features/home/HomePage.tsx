...
function formatDuration(startedAt: string, pausedMs: number): string {
  ...
  const elapsed = Math.max(
    0,
    Math.floor((Date.now() - start - pausedMs) / 1000),
  );
...
export default function HomePage({ onNavigate: _onNavigate }: Props) {
  ...
  const showTomorrowPrompt =
    !dismissedTomorrow && (planning?.suggest_tomorrow ?? false) && !session;
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <div
          className={`text-7xl font-bold tabular-nums ${isPositive ? "text-gradient-score-pos" : "text-gradient-score-neg"}`}
        >
          {isPositive ? "+" : ""}
          {totalDisplay}
        </div>
      ...
      {session ? (
        <div
          className={`border rounded-2xl p-6 mb-4 transition-all ${
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${session.state === "active" ? "bg-emerald-400 animate-pulse" : "bg-amber-400"}`}
                />
                {session.state === "active" ? "active" : "paused"}
              </span>
              {session.title && (
                <p className="text-zinc-300 font-medium mt-1.5">
                  {session.title}
                </p>
              )}
            </div>
          ...
          <div className="flex items-center justify-between mb-4 bg-black/20 rounded-xl p-3 border border-white/5">
            ...
            <p
              className={`text-2xl font-bold tabular-nums ${session.score_total >= 0 ? "text-gradient-orange" : "text-red-400"}`}
            >
        ...
        <div className="bg-zinc-900/60 border border-orange-500/20 rounded-2xl p-5 mb-4 card-glow-orange">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-sm font-semibold text-orange-300">
              plan today's quests first
            </p>
          </div>
          <p className="text-xs text-zinc-500 mb-4">
            add at least one quest to unlock session start
          </p>
          ...
          <div className="space-y-2 mb-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  playClick();
                  setQuestMainQuest(!questMainQuest);
                }}
        ...
        <div
          className="bg-zinc-900/60 border border-yellow-500/25 rounded-2xl p-5 mb-4"
          style={{
            boxShadow:
              "0 0 0 1px rgba(234,179,8,0.2), 0 0 20px rgba(234,179,8,0.07)",
          }}
        >
          <p className="text-base font-semibold text-zinc-100 mb-1">
            ☀️ did you get some sunlight?
          </p>
          <p className="text-xs text-zinc-500 mb-4">
            step outside before diving in — even 5 minutes counts
          </p>
          <div className="flex gap-2">
            <button
              onClick={async () => {
        ...
        <div
          className="bg-zinc-900/60 border border-emerald-500/25 rounded-2xl p-5 mb-4"
          style={{
            boxShadow:
              "0 0 0 1px rgba(52,211,153,0.15), 0 0 20px rgba(52,211,153,0.06)",
          }}
        >
          <p className="text-base font-semibold text-zinc-100 mb-1">
          ...
          <p className="text-xs text-zinc-500 mb-4">
            evening check-in — log it if you trained today
          </p>
          <div className="flex gap-2">
            <button
              onClick={async () => {
        ...
        <div className="bg-zinc-900/60 border border-white/5 rounded-2xl p-5 mb-4 text-center">
          <p className="text-zinc-400 mb-4">
            no active session. ready to play?
          </p>
          <button
            onClick={() => {
              playClick();
            ...
            <div className="flex items-center gap-2">
              {trackerStatus.is_idle ? (
                <span className="text-zinc-500 text-sm">
                  💤 idle ({Math.floor(trackerStatus.idle_seconds / 60)}m)
                </span>
              ) : (
                <>
          ...
          <div className="space-y-2">
            {plannedTasks.map((task) => (
              <div key={task.id} className="flex items-center gap-3">
                ...
                />
                <span
                  className={`text-sm ${task.is_main_quest ? "text-amber-300 font-medium" : "text-zinc-300"}`}
                >
                  {task.is_main_quest && "⭐ "}
                  {task.title}
                </span>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-zinc-300 font-medium">
                🌙 plan tomorrow before you go?
              </p>
              <p className="text-xs text-zinc-500 mt-0.5">
                you haven't added any quests for tomorrow yet
              </p>
            </div>
